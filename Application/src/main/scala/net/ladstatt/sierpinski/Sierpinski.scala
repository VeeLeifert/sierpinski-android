...
case class Triangle(var a:Pos, var b:Pos, var c:Pos, var color:SColor,var counter:Int = 6) {
  ...
  if (counter == 0) {
    ...
    siblings = List(Triangle(a, top, left, SColor(1.0, 0, 0), counter), Triangle(top, b, right, SColor(1.0, 0, 0), counter), Triangle(left, right, c, SColor(1.0, 0, 0), counter))
...
object Triangle {
  def apply(a: Pos, length:Int) = new Triangle(a, Pos(length,0), Pos(length/2,(Math.sqrt(3) / 2).toFloat*length),SColor(1.0,0,0) )
}
